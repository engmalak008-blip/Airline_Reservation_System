...
int checkSeatStatus(string seat, Flight f) {
    if(seat.length() < 2 || seat.length() > 5) return 1;

    seat = normalizeSeat(seat);

    if(seat[0] != 'A') return 1;
    ...
    if(num < startSeat || num > endSeat) return 2;
...
public:
    ...
    string generateTicketID() {
        return "TKT" + to_string(1000 + passengersCount);
    }
    ...
    bool isSeatTaken(int flightId, string seat) {
        for(int i = 0; i < passengersCount; i++) {
            if(passengers[i].flightId == flightId && passengers[i].seatNumber == seat){
                return true;
            }
        }
    ...
    void inputPassengerData(int index, int flightId) {
        passengers[index].name = getStringInput("Name: ");
        passengers[index].age = getIntInput("Age: ");
        passengers[index].flightId = flightId;
        passengers[index].ticketID = generateTicketID();
    }
    ...
    string chooseSeat(Flight f) {
        ...
        while(true) {
            cout << "Available Seats (A"<< startSeat<< " - A"<< endSeat<< "): ";
    ...
    void addFlight() {
        ...
        if(existingFlight != -1) {
            ...
            if(choice == 1) {
                ...
                flights[existingFlight].seatOffset =calculateSeatOffset( flights[existingFlight].from, flights[existingFlight].to, existingFlight);
        ...
        flights[flightCount].id = id;

        flights[flightCount].from = getStringInput("From: ");

        flights[flightCount].to = getStringInput("To: ");
        ...
        flights[flightCount].totalSeats =seats;
        flights[flightCount].availableSeats =seats;
        flights[flightCount].seatOffset = calculateSeatOffset(flights[flightCount].from,flights[flightCount].to,flightCount);
        flightCount++;
    ...
    void suggestAlternativeFlights(string fromCity,string toCity,int requiredTickets){
        ...
        for(int i = 0; i < flightCount; i++) {
            if(flights[i].from == fromCity && flights[i].to == toCity && flights[i].availableSeats >= requiredTickets){
                if(!found) {
                    cout << "\nSuggested Flights:\n";
                    found = true;
                }
    ...
    void bookTicket() {
        ...
        if(passengersCount + tickets > 100) {
            cout << "❌ System Full\n";
            return;
        }
        ...
        if(tickets >flights[flightIndex].availableSeats){
            ...
            suggestAlternativeFlights(flights[flightIndex].from,flights[flightIndex].to,tickets);
            return;
        }
        ...
        for(int i = 0; i < tickets; i++) {
            ...
            inputPassengerData(passengersCount,id);

            passengers[passengersCount].seatNumber = chooseSeat(flights[flightIndex]);

            cout << "✔ Ticket Booked: "
                 << passengers[passengersCount].ticketID
                 << endl;
    ...
    void cancelBooking() {
        ...
        for(int i = 0; i < passengersCount; i++) {
            ...
            if(passengers[i].ticketID == ticket && passengers[i].flightId == flightId && passengers[i].seatNumber == seat){
                flights[flightIndex].availableSeats++;

                for(int j = i;j < passengersCount - 1;j++){
